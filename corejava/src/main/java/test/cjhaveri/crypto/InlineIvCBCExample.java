...
/**
 * Symmetric encryption example with padding and CBC using DES with the
 * initialization vector inline.
 */
public class InlineIvCBCExample {
	public static void main(String[] args) throws Exception {
		...
		byte[] originalIV = new byte[ivBytes.length];

		System.arraycopy(buf, 0, originalIV, 0, ivBytes.length);

		System.out.println("plain : " + Utils.toHex(plainText, plainText.length) + " bytes: " + plainText.length);

		System.out.println("IV bytes: " + Utils.toHex(ivBytes) + "; Original IV: " + Utils.toHex(originalIV));
